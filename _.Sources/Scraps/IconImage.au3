; https://www.autoitscript.com/forum/topic/133053-iconimage-udf/

; =============================================================================
;  AutoIt IconImage UDF (2017.11.21)
;  Purpose: Icon Management And Conversion (Among IconImage/hIcon/.ico)
;  Author: Ward
;  Modified: KaFu > made AutoIt Version 3.3.14.2 compatible
;  Required:
;	"Binary.au3"		by Ward
; =============================================================================
...
Func _IconImage_FromIcoData($IconData, $Width, $Height = Default, $Index = Default)
	...
	EndIf
EndFunc   ;==>_IconImage_FromIcoData
...
EndFunc   ;==>_IconImage_FromIcoFile
...
Func _IconImage_FromImageData($ImageData)
	...
	Return SetError($Error, 0, $IconImage)
EndFunc   ;==>_IconImage_FromImageData
...
Func _IconImage_FromImageFile($FileName)
	...
	Return SetError(@error, @extended, $Ret)
EndFunc   ;==>_IconImage_FromImageFile
...
Func _IconImage_FromGdipBitmap($Gdibmp)
	...
	Do
		...
		Local $Header = DllStructCreate(StringReplace($tagBITMAPINFO, "dword biRGBQuad[1]", ""))
...
Func _IconImage_ToIcoFile($IconImage, $FileName)
	...
	If Not @error Then
		...
		FileClose($File)
	EndIf
EndFunc   ;==>_IconImage_ToIcoFile

Func _IconImage_ToImageData($IconImage, $Type = "PNG", $iQuality = 92)
	Local $Gdibmp, $Error = 1, $ImageData = Binary("")
	Do
		$Gdibmp = _IconImage_ToGdipBitmap(Binary($IconImage))
		If @error Then ExitLoop

		$ImageData = _IconImage_SaveGdipBitmapToBinary($Gdibmp, $Type, $iQuality)
		$Error = @error
	Until 1
	If $Gdibmp Then _GDIPlus_ImageDispose($Gdibmp)
	Return SetError($Error, 0, $ImageData)
EndFunc   ;==>_IconImage_ToImageData
...
Func _IconImage_ToImageFile($IconImage, $FileName, $Type = "PNG")
	...
	If Not @error Then
		...
		FileClose($File)
	EndIf
EndFunc   ;==>_IconImage_ToImageFile
...
Func _IconImage_ToGdipBitmap($IconImage)
	...
	Else
		...
		Return $Gdibmp
	EndIf
EndFunc   ;==>_IconImage_ToGdipBitmap
...
EndFunc   ;==>_IconImage_ConvertToBMPIcon
...
Func _IconImage_ConvertToPNGIcon($IconImage)
	...
	If $Gdibmp Then _GDIPlus_ImageDispose($Gdibmp)
	Return SetError($Error, $IsConvert, $NewImage)
EndFunc   ;==>_IconImage_ConvertToPNGIcon

Func _IconImage_GetSize($IconImage)
	Local $Error = 0, $Width = 0, $Height = 0, $BitCount = 0, $Header

	If _IconImage_IsPNGIcon($IconImage) Then
		$Header = DllStructCreate("byte[16];byte biWidth[4];byte biHeight[4]")
		_BinaryFillDLLStruct($IconImage, $Header)

		; convert network byte order
		$Width = Dec(StringTrimLeft(DllStructGetData($Header, "biWidth"), 2))
		$Height = Dec(StringTrimLeft(DllStructGetData($Header, "biHeight"), 2))
		$BitCount = 32
...
EndFunc   ;==>_IconImage_FromGroupId
...
Func _IconImage_CreateStream($InitData = Default)
	...
	Return SetError(1, 0, 0)
EndFunc   ;==>_IconImage_CreateStream
...
EndFunc   ;==>_IconImage_ReleaseStream

Func _IconImage_SaveGdipBitmapToBinary($Gdibmp, $Type = "PNG", $iQuality = 92)
	Local $Error = 1, $Binary = Binary(""), $Stream = 0, $GUID
	Do
		...
		Switch $Type
			Case "JPG"
				$GUID = _WinAPI_GUIDFromString(_GDIPlus_EncodersGetCLSID($Type))
...
EndFunc   ;==>_IconImage_SaveGdipBitmapToBinary
...
Func _IconImage_LoadGdipBitmapFromBinary($Data)
	...
	If $Stream Then _IconImage_ReleaseStream($Stream)
	Return SetError($Error, 0, $Gdibmp)
EndFunc   ;==>_IconImage_LoadGdipBitmapFromBinary
...
Func _IconImage_32BitAlphaToGdipBitmap($IconImage)
	...
	Return SetError($Error, 0, $Gdibmp)
EndFunc   ;==>_IconImage_32BitAlphaToGdipBitmap
...
EndFunc   ;==>_IconImage_CreateMaskBmp
...
EndFunc   ;==>_IconImage_VistaAndLater
